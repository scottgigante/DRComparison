...
if __name__ == '__main__':
	...
	parser.add_argument('--normcounts_file', metavar='normcounts_file', default=None, required=True, help='normcounts_file file')
	...
	normcounts = np.loadtxt(args.normcounts_file)
	normcounts = np.transpose(normcounts)
	normcountstable = np.array(normcounts)
	start_time = time.time()
	Z, model_params = fitModel(normcountstable, args.num_pc)
	elapsed_time = time.time() - start_time

	np.savetxt(args.out, Z, fmt='%.4f')
