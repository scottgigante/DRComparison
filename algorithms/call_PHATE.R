...
# main function
call_PHATE <- function(sce, num_pc, params){
	...
	tryCatch({
		ct <- system.time({
		data_phate <- phateR::phate(norm_counts, ndim=num_pc, n.jobs=20)
		data_phate <- as.matrix(data_phate)
		})	
		# count time
		ct <- c(user.self = ct[["user.self"]], sys.self = ct[["sys.self"]], 
            user.child = ct[["user.child"]], sys.child = ct[["sys.child"]],
            elapsed = ct[["elapsed"]])
		list(res = data_phate, ctimes = ct)
	},
	error = function(e) {
